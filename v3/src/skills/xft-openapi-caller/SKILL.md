...
description: 通过 xft-gateway 的 interfaces 列表选择可调用接口，使用返回的 docid 抓取目标文档，准备并通过 xft-gateway 执行 API 调用，并安全地总结结果。适用于需要查询或调用 XFT HTTP API 的场景，例如组织查询、差旅申请创建、申请单详情查询等。
---
# XFT OpenAPI 调用技能
...
- 使用 `xft-cli api interfaces` 查看当前 token 有权限调用的网关接口名；中间层返回的 `docid` 是后续文档抓取入口
- 使用 `xft-cli doc fetch --docid <docid>` 将目标文档抓取到 `~/.xft_config/xft_docs`，并优先复用已有缓存
...
- 不再通过本地文档搜索命令选择接口；接口选择应以 `xft-cli api interfaces` 返回的接口列表和 `docid` 为准
...
3. 处理网关认证。
   ...
   - 如果已通过 `XFT_GATEWAY_BASE_URL` 和 `XFT_GATEWAY_TOKEN` 提供配置，可跳过交互式认证。
4. 查询当前账号有权限调用的接口列表。
   - 执行：`xft-cli api interfaces`
   - 从返回的 `data[]` 中选择最符合用户需求的接口，记录 `name` 作为 `interface_name`，记录 `docid` 作为文档编号。
   - 如果列表中没有目标接口，停止并说明需要管理员先在网关维护接口目录和权限。
   - 如果目标接口缺少 `docid`，停止并说明需要管理员补齐该接口的文档编号。
5. 用接口列表返回的 `docid` 抓取文档。
   - 示例：`xft-cli doc fetch --docid 12509`
   - 命令会直接输出缓存 markdown 目录路径。
   - 需要强制刷新时加 `--force`。
   - 只有在确实需要时才加 `--html` 或 `--raw`；默认会生成 markdown。
6. 从文档中提取接口契约，并和网关接口名对应起来。
7. 如果是差旅场景，自动解析城市编码。
...
- 先说明你选择了哪份文档、为什么选它。
- 说明使用的网关接口名、接口列表返回的 `docid`，以及它和文档接口的对应关系。
