...
import type { BaseReqInf, LoginTokenInfo, TokenLoginInfo, XftFrameworkResponse } from "./types.js";

function decodeBase64Data(data: string): string {
...
export function getDataParamMap(dataStr: string): Record<string, string> {
...
function verifyDataAndSign(dataStr: string, sign: string, authoritySecret: string): void {
...
function parseFrameworkResponse<T>(responseBody: string): XftFrameworkResponse<T> {
  return JSON.parse(responseBody) as XftFrameworkResponse<T>;
}
...
export class XftVerifyTokenClient {
  constructor(
    ...
    private readonly loginUserUrl?: string,
  ) {}

  async verifyToken(data: string, sign: string): Promise<LoginTokenInfo> {
    ...
    const reqInf: BaseReqInf = { ...this.baseReqInf };
    ...
    const framework = parseFrameworkResponse<LoginTokenInfo>(responseBody);
  ...
  async getLoginInfo(data: string, sign: string): Promise<TokenLoginInfo> {
    ...
    const responseBody = sm4DecryptEcb(this.baseReqInf.authoritySecret.slice(0, 32), response.body);
    const framework = parseFrameworkResponse<TokenLoginInfo>(responseBody);
    if (framework.returnCode !== Constants.SUCCESS_CODE) {
      throw new XftSignException(framework.returnCode, framework.errorMsg ?? "未知错误");
    }
    return framework.body;
