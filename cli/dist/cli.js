...
import { XftOpenApiReqClient, XftVerifySignClient, XftVerifyTokenClient, sm4DecryptEcb, sm4EncryptEcb, } from "./index.js";
...
async function main() {
    ...
    if (command === "feature-call") {
        ...
        if (requestMode === "upload") {
            ...
            const result = feature.useOriginalName || options["use-original-name"]
    ...
    if (command === "post") {
        ...
        const result = await XftOpenApiReqClient.doCommonPostReq(reqInf, requireOption(options, "url"), parseJsonOption(options, "query-json"), body);
        printJson(result);
        return;
    }
    if (command === "get") {
